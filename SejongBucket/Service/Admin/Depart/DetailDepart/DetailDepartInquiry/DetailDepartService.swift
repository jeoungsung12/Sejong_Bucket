...
class DetailDepartService {
    static func requestInquiry(majorId : DetailDepartModel) -> Observable<DetailDepartServiceModel> {
...
//            })
//            .dispose()
            let url = "http://ime-locker.shop:8083/master/api/v2/majors/\(majorId.majorId)/majorDetails"
            if let accessToken = KeychainWrapper.standard.string(forKey: "JWTaccessToken"){
                AF.request(url, method: .get, encoding: JSONEncoding.default ,headers: ["AccessToken" : "\(accessToken)"])
                    .validate()
                    .responseDecodable(of: DetailDepartServiceModel.self) { response in
