//
//  AddDetailDepartViewController.swift
//  SejongBucket
//
...
class AddDetailDepartViewController : UIViewController {
    private let disposeBag = DisposeBag()
    private let addDetailDeparttViewModel = AddDetailDepartViewModel()
    //이니셜 라이저
...
//MARK: - 레이아웃
extension AddDetailDepartViewController {
    func setLayout() {
...
extension AddDetailDepartViewController {
    func setBinding() {
        makeBtn.rx.tap
            .subscribe(onNext: {[weak self] in
                self?.addDetailDeparttViewModel.nameInputTrigger.onNext(DetailProduceModel(majorDetailName: (self?.departText.text)!, majorId: (self?.major.id)!))
            })
            .disposed(by: disposeBag)
        addDetailDeparttViewModel.outputResult
    ...
    private func moveToDepartManagement() {
        ...
        navigationController?.viewControllers.removeAll(where: { $0 is AddDetailDepartViewController })
    }
}
extension AddDetailDepartViewController: UITextFieldDelegate {
